...
-- SELECT ~ DDL : 테이블
-- TABLE 테이블 : 데이터베이스에서 사용하는 기본 객체
--INDEX : 검색하는 속도를 빠르게 만들어주는 객체 
--INDEX를 따로 저장할 별도의 저장공간 필요 
--DML작업이 자주 일어나면 오히려 성능을 저하시킴
--요새는 컴사양이 좋아서 검색속도가 다빠름 
--중요하기 때문에 나중에 따로라도 공부하시길  ←...?
--
--고유인덱스 : UNIQUE / PRIMARY KEY 제약 조건 넣을 시 자동으로 생성
