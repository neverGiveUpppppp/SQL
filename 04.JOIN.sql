...
다중조인 이해가 안될 때,
조인을 하나씩 결과를 찍어서 그림판에 넣어놓고
또 다른거를 붙이고 하는 식으로 어떤 행이 어떻게 이어지는지 한번에 볼 수 있음
...
1.내부조인(Inner join)
    1)오라클 전용 구문
    2)ANSI 표준 구문 : ANSI 미국 표준 협회(American National Standards Institute)
2.외부조인(Outer join)

B.JOIN 사용 가능 조건
두 테이블 안에 컬럼명이 다르더라도 데이터값이 같은 공통분모가 있다면 사용가능

C.설명
컬럼을 고리 역할로 해서 A와 B테이블을 연결한다
A,B 무조건 같은 컬럼명이 있어야 하나? NO
접점을 찾아라
DEPT_ID 값과 DEPT_CODE값 <- 둘의 내부 데이터가 D1-D9으로 같다
각 테이블 안에 같은 데이터를 가지고 있으면 두 테이블은 조인할 수 있다



D.JOIN 실행순서?
FROM절에서 쓰이기 때문에 FROM절 실행시 같이 실행됨  
...
< 비등가 조인 >
크다 작다 > < 어떤 범위 안에 들어가 있게끔 하는게 비등가 조인

만약 EMPLOYEE TABLE에 SAL_GRADE.SAL_LEVEL이 없다면?
...
-- 오라클 전용 구문
-- 풀이 과정
--SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE -- 둘 다 사용가능                    
SELECT EMP_ID, EMP_NAME,DEPARTMENT, DEPT_ID, DEPT_TITLE -- DEPT_ID, DEPT_CODE 둘다 부서코드를 나타내니 뭘 쓰던 상관X
FROM EMPLOYEE, ; -- 두 테이블을 조인할거기에 두테이블 명시
-- 결과 : D1-9까지 모든 사원이 합쳐져 나와서 결과값이 207개나 나옴
-- 조건없이 D1에 있는거 다 D2에 있는거 뽑아오라고 해서 이런 결과가
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE, JOB
WHERE JOB_CODE = JOB_CODE; -- ERROR : ORA-00918: column ambiguously defined


SELECT EMP_ID, EMP_NAME, EMPLOYEE.JOB_CODE/*JOB.JOB_CODE도 가능*/, JOB_NAME -- ERROR :ORA-00918: column ambiguously defined
FROM EMPLOYEE, JOB                          -- JOB_CODE가 2개라 어떤걸 가져와야할지 모르겠다는 것
WHERE EMPLOYEE.JOB_CODE = JOB.JOB_CODE; 
-- 컬럼명이 같으면 어느 테이블 컬럼인지 지정해줘야함 : 테이블명.컬럼명으로 지정
-- ex) EMPLOYEE.JOB_CODE // 
...
즉 EMPLOYEE가 기준
디파트가 임플한테 맞춰주는 것. 
그래서 오라클 전용구문에서 표기가 기준점이 되는 테이블이 아닌 조인하는 테이블(+) 붙임
 -> (+)가 안붙은 쪽의 컬럼 쪽 데이터 다 가져온다
