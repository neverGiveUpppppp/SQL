...
SET SERVEROUTPUT ON; 
-- 실행하려는 쿼리가 참조하는 테이블이 있는 계정으로 접속하여 변경해줘야 한다.
BEGIN
...
SALARY 3500000
*/
-- WHERE절 조건을 DEPT_CODE로하고 D9를 스캐너에 입력한 경우 
-- exact fetch returns more than requested number of rows
-- exact fetch가 요청된 행 수보다 많은 행을 반환합니다
...
-- 한 행 전체를 가지고 오는 레퍼런스
-- 변수명 참조할테이블명%ROWTYPE;
-- select 결과를 into문에 지정한 곳에 넣어주는 것
...
BEGIN             
    ...
    -- YSALRY := VEMP.SALARY * (1+ NVL(VEMP.BONUS, 0)) * 12; -- 다른 풀이방식
    END IF;
...
-- 버퍼 오버플로, 1000000 바이트 제한
--    → 반복문 나갈 수 있는조건 설정X 해서
...
-- 1..5 → 1부터 5까지 .. 점 두개
-- 그럼 N은 자동적으로 NUMBER 타입으로 변수설정 되는건가요? O


-- 1~5까지 순차적 출력
BEGIN
    FOR N IN 1..5   -- 의미 1부터 5까지 // .. 2개 점을 3개쓰면 에러발생
            ...
            THEN
                ...
                --LOOP     
                    ...
                    LOOP
                        DBMS_OUTPUT.PUT_LINE(N||' x '||M||' = '||N*M );
                    END LOOP;
                --END LOOP; 아래 정답과 차이는 주석처리된 LOOP END LOOP가 있고 없고 차이
        END IF;
    END LOOP;
END;
/
-- FOR문으로 구구단 출력(짝수단만 출력)
BEGIN
    FOR I IN 2..9
        LOOP
            IF(MOD(I,2) = 0)  THEN
...
-- 예외 처리(EXCEPTION)
-- 시스템에 있는 예외가 있고 사용자정의 예외도 따로 만들 수 있음

-- 예외 메세지
-- NO_DATA_FOUND : SELECT문이 데이터 행을 반환하지 못할 때,  발생하는 예외
-- DUP_VAL_ON_INDEX : UNIQUE 제약조건이 들어간 컬럼에 중복 값이 들어갔을 때,
--                      DUPLICATE VALUE ON INDEX
-- ZERO_DIVIDE : 0으로 나눌 때 발생하는 예외
...
BEGIN
    ...
    -- 예외처리 해보자
    -- EMPLOYEE테이블에 EMP_ID 201인 사원이 이미 존재하는데 EMP_ID가 201인 사원을 
    -- 업데이트 해주겠다고하면 에러가 발생 EMP_ID 201인 사원이 이미 존재하기 때문에 에러가 발생한다.
EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
        DBMS_OUTPUT.PUT_LINE('이미 존재하는 사번입니다.');
-- 해당 예외가 발생했을때에 출력되는 구문을 '이미 존재하는 사번입니다.'으로 바꿔준다.            
END;
/
