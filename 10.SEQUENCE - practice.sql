...
-- 테이블 안 데이터 삭제
DELETE FROM UC; -- 해당 테이블의 데이터 전체삭제 
DELETE FROM UC  -- 해당 테이블의 USER_NO컬럼의 값이 5인 데이터 행만 삭제
WHERE USER_NO = 5;
...
WHERE USER_NO = 2;
UPDATE UC
SET NATIONALITY = DEFAULT -- 한국이 디폴트 세티이었던 것 같은데 안먹힘
WHERE USER_NO = 1;
UPDATE UC
SET NATIONALITY = '한국' -- 적용완료
WHERE USER_NO = 1;


-- 실험 툴
DROP SEQUENCE SEQ_FIRST2;
SELECT * FROM UC;
...
SELECT * FROM P_EC2;
SELECT * FROM UC;
INSERT INTO UC VALUES(2, 'B','BB','남','미국');
INSERT INTO UC VALUES(SEQ_EMPID.NEXTVAL,'C','CC','남',DEFAULT); -- 41	C	CC	남	미국
INSERT INTO UC VALUES(SEQ_EMPID.NEXTVAL,'D','DD','남',DEFAULT); -- 51	D	DD	남	미국
INSERT INTO UC VALUES(SEQ_FIRST.NEXTVAL,'E','EE','남','호주');
INSERT INTO UC VALUES(SEQ_FIRST2.NEXTVAL,'F','FF','남','호주'); -- 맥스인 5까지 INSERT문 실행할 때마다 1증가

CREATE SEQUENCE SEQ_FIRST2
MAXVALUE 5
CYCLE
NOCACHE;
...
-- 시퀸스 수정
ALTER SEQUENCE SEQ_FIRST 
START WITH 1 -- ERROR : 수정으로 시작값은 변경 불가
INCREMENT BY 1
MAXVALUE 5
MINVALUE 0;
