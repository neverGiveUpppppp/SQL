...
-- 엄청나게 긴 쿼리문이 생기게 됩니다. 저장된 프로시저를 호출하여 프로그래밍을 하는것이 훨씬 효율적입니다.
...
END;
/
-- 차이 : 아래에서는 PROCEDURE생성하고 그 안에서 삭제함.
...
DEL EMP ID라는 프로시져에 200이 들어가있는 상태로 컴파일이 되어버림
    -> 프로시저 실행할 때, BEGIN부분에서 내용이 고정되서 코드가 실행된다는 것
         그래서 매개변수를 써줘야한다
...
-- OUT 매개변수는 바인딩변수를 선언하고 이 바인딩 변수를 PROCEDURE 매개변수랑 연결해서 사용하는 개념
-- PROCEDURE 옆에 소괄호인 매개변수와 EXECUTE의 매개변수의 순서와 수에 맞춰서 써야한다
-- EXECUTE 후 PRINT 실행시 해당하는 사번의 해당 내용 출력
...
IS                                        
BEGIN
    SELECT EMP_NAME,SALARY,NVL(BONUS,0) -- 셀렉트한 결과를 INTO에 담에놓고 위에 아웃에서 쓰겠다는 것
    INTO EMP_NAME,SALARY,BONUS
    FROM EMPLOYEE
    WHERE EMP_ID = V_EMP_ID/*변수명*/; -- V_EMP_ID : 안에서 실행되서 IN 매개변수에 포함됨
END;
...
-- 참조 // 콜론 꼭 붙일 것
-- EXECUTE 후 PRINT 실행시 해당하는 사번의 해당 내용 출력
-- 코드 양식 : (아웃매개변수 순서와 수에 맞춰서) :바인드 변수 
-- IN 매개변수 굳이 IN 쓰는이유 IN 과 OUT 구분
...
-- FUNCTION과 PROCEDURE를 출력시키기 위해서는 바인드 변수에 해당 데이터를 := 코드를 사용하여
-- 실행시키고 PRINT 명령어를 통해 출력해야한다

-- 216번 사원의 이름 출력
CREATE OR REPLACE FUNCTION SELECT216
RETURN EMPLOYEE.EMP_NAME%TYPE -- 뭘 반환할껀지 리턴 데이터타입을 정해줘야함
...
END; -- Function SELECT216이(가) 컴파일되었습니다.
/
