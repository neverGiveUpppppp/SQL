...
--시퀀스 SEQ_EMPID.이 세션에서 CURRVAL이 아직 정의되지 않았습니다.

SELECT SEQ_EMPID.NEXTVAL FROM DUAL;
--3번 실행시 에러 발생
-- 시퀀스 SEQ_EMPID.NEXTVAL exceeds MAXVALUE은 사례로될 수 없습니다.
-- 3번실행 시 MAXVALUE 초과 → 에러발생
-- CYCLE로 설정시 초기값으로 다시 넘어갔을 것

-- 시퀀스 SEQ_EID 생성
-- 단 시작 값은 300이고 최대값은 10000, 순환하지 않고 캐시메모리 사용 안하며 증가 숫자는 1
...
-- MAXVALUE는 현재 수치보다 작은 수치로는 변경불가
