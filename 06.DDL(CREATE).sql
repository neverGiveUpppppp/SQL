...
DQL은 테이블 내의 데이터를 조회할 때 사용하는 SQL문언어의 유형
...
--데이터 타입
--문자 : CHAR(크기(바이트/문자)), VARCHAR2(크기(바이트/문자)), NVARCHAR, LOB(CLOB/BLOB) :CHAR VACHAR 둘 다 한글자 이상 담을 수 있다.  LOB 많은 글자수
--크기에 BYTE만 들어갈 수 있는 것아님,
-- 한글 3 BYTE, 영어 숫자 : 1BYTE
--      CHAR : 고정 문자열 자료형 (최대 2000byte)
--      VARCHAR2 : 가변(지정한 크기 이내의) 문자열 자료형(최대 4000byte)
--      ex. CHAR(10): 바이트 / CHAR(10Char): 문자           
--          한글 3글자         한글 10글자
--      ex. CHAR(10Char)     VARCHAR2(10Char)
--          안녕하세요_ _ _ _ _  안녕하세요
--          뒤에 남은 5개 자리 유지 / 뒤의5개 공간 없음 (10Char = 10개의 글자공간)
--  지정크기보다 적게 입력되면 남은자리 공백입력 / 지정크기보다 적게입력되면 남은자리 없앰
--          NVARCAHR : 문자 개수 자체를 길이로 계산하는 자료형 → 유니코드에 적합

--숫자 : NUMBER[(P, S)]
--      NUMER : 최대 40자리
--      NUMBER(크기) : 길이 지정 가능
--         P : 최대 자리수 / S : 소수점 자리수
--      ex. NUMER(4,3) → 최대 4자리 수, 소수점 3자리 허용
...
-- 예약어 : USER_CONSTRAINTS


-- 예약어 : USER_CONS_COLUMNS
-- 제약조건, 테이블명, 컬럼명, 포지션 다 보여주는 시스템명령어
...
FROM USER_CONSTRAINTS C1  
    JOIN USER_CONS_COLUMNS C2 USING(CONSTRAINT_NAME)
WHERE C1.TABLE_NAME = 'SYS_C007021';
