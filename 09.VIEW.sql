...
-- 뷰는 실질적 데이터 저장 X SELECT 결과화면 저장한 객체
-- TEXT에 VIEW 정의 쿼리 그대로 들어가 있음
--뷰 실행시 TEXT 에 저장되어 있는 쿼리 내용 그대로 실행
-- 그런데, 어차피 DML(INSERT, UPDATE, DELETE) 를 해보았자 TEXT에 SELECT된 것은 FROM 본테이블이므로 
--본 테이블에 영향이 간다.
-- 본테이블 과 뷰는 서로 영향을 미친다.
--TEXT안에 FROM JOB(본테이블과 연결된 부분)이라고 적힌 부분 때문에 본테이블과 연결되는 것 맞나요?  O
--VIEW에서는 따로 TEXT에서 코드를 실행해서 사람에게 보여주는데 
--이 TEXT 코드에 안에 본테이블인 JOB의 데이터를 참조해서 끌어와 쓰기 때문에 
--본테이블과 뷰 서로 영향이 있다는 것 같습니다
--참조라는 표현이 정확한지는 모르겠으나 비슷한 개념인 것 같아요  O
...
-- 원본테이블에서 복사해온 뷰객체 테이블은 DML이 가능하나,
-- 원본 데이터가 다르게 무언가 가공된 데이터들은 DML이 불가능
--      ex)원본 데이터랑 다르게 뷰객체에 DISTINCT를 해서 가져온다던가, 
--         다른 컬럼을 가져온다던가 하면 DML 사용불가
...
-- 키가 보존되지 않은 테이블에 매핑되는 열은 수정할 수 없습니다
...
OR REPLACE : 기존의 동일한 이름의 뷰가 존재하면 덮어쓰고, 존재하지 않으면 새로 생성하는 옵션
FORCE : 서브쿼리에 사용된 테이블이 존재하지 않아도 뷰 생성
NOFORCE : 기본값, 서브쿼리에 사용된 테이블이 존재해야만 뷰 생성
WITH CHECK OPTION :옵션으로 설정한 컬럼의 값 수정 불가능
WITH READ OPTION : 뷰에 대해 조회만 가능(DML 불가능)
...
FROM EMPLOYEE;        
--ORA-00955: name is already used by an existing object
--이름이 기존 개체에서 이미 사용되고 있습니다.
-- OR REPLACE 옵션이 없기 때문에 존재하는 이름일 때는 덮어쓸 수 없음
...
-- 뷰를 통해 볼 수 없는 행을 업데이트하거나 삽입할 수 없습니다
