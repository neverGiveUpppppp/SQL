...
2)WHERE 통해서 조건 추려내고
보고 싶은 컬럼들만 취함
3)GROUP BY로 그룹함수가 적용될 그룹을 묶어주고 
4)HAVING
5)SELECT
6)ORDER BY
*/
...
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
-- 샐러리 300이상인 사원만 테이블 EMPLOYEE에서 전부 뽑고난 후, 그룹바이로 DEPT_CODE로 부서별로 그룹핑해서
-- 샐러300만이상 사원이 있는 부서만 보여주고, 해당 부서의 평균 샐러리 AVG(SALRARY)를 보여준다
...
-- DEPT 코드별로 평균을 냈을 때 300000원이 넘는 부서만 그룹핑함.
...
-- 롤업은 첫번째 dept_code 부서별 합계 한번 구하고, 마지막에 전체합계 구하고
-- 큐브는 롤업이랑 같은데 추가로 job_code의 잡코드별 합계를 한번 구하고 전체합계를 구함
-- 인자 두가지에 대해 다 하는게 큐브고, 롤업은 인자 두가지 중 앞에꺼만 적용
