...
-- 'ABC' 묶여진 문자 하나로 보는게 아니라, A 또는 B 또는 C가 있으면 각각 다 지우는 것
-- 'ABC' 안에 순서 상관X
...
SELECT SUBSTR('HELLOMYGOODFRINEDS', -8, -3) FROM DUAL; -- (null) -는 안먹힘
...
--SELECT EMP_NAME, REPLACE(EMP_NO, SUBSTR(EMP_NO, INSTR(EMP_NO, '-')+1), '********') 주민번호 FROM EMPLOYEE;
...
-- mod의 앞 숫자의 기호를 따라감. ex) 10이 +면 +로 따라가고 -10면 -를 따라감
...
-- 양수에서의 내림은 다 같고, 음수에서 차이난다. 
-- 차이 trunc는 0쪽으로 절삭, floor는 그냥 내림
...
-- 질의결과에서 숫자로 인지되었으면 오른쪽 정렬, 문자로 인지되어있으면 왼쪽 정렬
...
SELECT TO_CHAR(SYSDATE, 'YY-MM-DD DY') FROM DUAL;  -- 22-03-20 일
...
SELECT TO_CHAR(SYSDATE, 'YYYY-FMMM-DD DAY HH:MI:SS') FROM DUAL; --2022-3-20 일요일 3:48:47 공백,0 제거
-- FM 붙이는 순간 0으로 표ㅛ시되는 공백이 모두 제거됨
...
SELECT TO_CHAR(SYSDATE,'MM'), TO_CHAR(SYSDATE,'MONTH'),
        TO_CHAR(SYSDATE,'MON'), TO_CHAR(SYSDATE,'RM') -- RM : 로만숫자 
...
FROM DUAL; -- 079	20	1
-- 1월 1일부터 세기 시작해서 지정 날짜에 몇일이 지났는가
SELECT TO_CHAR(SYSDATE,'Q'), TO_CHAR(SYSDATE,'DAY'), TO_CHAR(SYSDATE,'DY')
FROM DUAL;
-- Q : 분기를 나타냄. 1-4분기
-- DAY : 요일 글자까지 출력. 월요일 화요일 수요일
-- DY : 요일 글자빼고 월수목금 식으로 출력
...
SELECT EMP_NAME, JOB_CODE, SALARY, 
    ...
    -- DECODE(적용할 컬럼명 , 조건식1, 선택값1, 조건식2, 선택값2 ...)
FROM EMPLOYEE;
...
SELECT MIN(SALARY), MAX(SALARY), EMP_NAME FROM EMPLOYEE; 
-- →not a single-group group function 단일함수와 일반 그룹항의 행의 갯수가 맞지 않아서
-- 1행 VS 23행
