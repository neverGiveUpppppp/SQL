...
CREATE TABLE USER_TEST(
    ...
    USER_ID VARCHAR2(20) CONSTRAINT UQ_UT_USERID UNIQUE,
    USER_PWD VARCHAR2(20) CONSTRAINT NN_UT_USERPWD NOT NULL, -- 
    PNO VARCHAR2(20)CONSTRAINT NN_UT_PNO NOT NULL, -- 제약조건 2개적용이라 테이블 레벨 이용할 것
    GENDER VARCHAR2(3)CONSTRAINT CK_UT_GENDER CHECK(GENDER IN('남','여')), -- GENDER에 남과 여만 들어가게 제약조건 생성
    PHONE VARCHAR2(20),
    ADDRESS VARCHAR2(100),
    STATUS VARCHAR2(3) CONSTRAINT NN_UT_STATUS NOT NULL,
    CONSTRAINT UQ_UT_PNO UNIQUE(PNO), -- 테이블 레벨에서는 제약 조건 한번에 하나만 기술 가능한 듯?
    CONSTRAINT CK_UT_STATUS CHECK(STATUS IN('Y','N'))
);    

-- 제약조건 컬럼레벨에서 2개씩도 사용가능
-- 사용법 : CONSTRAINT NN_UT_USERPWD NOT NULL CONSTRAINT NN_UT_USERPWD NOT NULL,
-- CONSTRAINT 두번쓰면 되고 CONSTRAINT 사이에 콤마(,)없이 띄어쓰기로 해야함
