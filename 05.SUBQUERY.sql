...
-- 서브쿼리는 WHERE 절 뿐만 아니라 HAVING 절에도 들어 갈 수 있다.(다른 절도 가능)
...
--2)17700000급여 합계를 갖고 있는 부서명, 급여합계
...
-- 다중행 서브쿼리 : 일반 연산자 사용 X
-- IN/NOT IN : 여러개 결과 값 중 하나라도 일치하는 값이 있다면/없다면
--SELECT EMP_NAME
--FROM EMPLOYEE
--WHERE EMP_ID IN(200, 201, 202, 205);
-- > ANY, < ANY : 여러개의 결과 값 중 하나라도 큰/작은 경우 →가장 작지않다 , 가장 크지않다
-- > ALL, <ALL  : 모든 값보다 큰/작은 경우→ 최대값 /최소값
-- EXISTS / NOT EXISTS : 값이 존재하는/ 존재하지 않는 경우
...
SELECT에서 ROWNUM과 함께 *를 조회하고 싶을 경우
FROM절 테이블명에 별칭을 설정하고 별칭.*로 조회 가능하다
...
-- 인라인뷰안에서 EMP_NAME을 별칭을 써서 이름으로 바꾼거고, 외부 SELECT에서는 *에서는 전체조회하는데
-- 여전히 EMP_NAME으로 검색하니까 오류남. 별칭 때문에 이름이 바뀌어 있는 걸 EMP_NAME으로 조회했으니 에러
...
------------------------------------------------------------------------------
/*
-- RANK() OVER / DENSE_RANK() OVER  : 순위를 매기는 함수
...
-- ROWNUM : FROM절 (판)이 깔리고나서 번호를 매김
-- 실행순서 : 1. FROM  2. WHERE 3. SELECT 4.ORDER BY
-- ROWNUM이 5번 이하인 사람들 을 기준으로 정렬 (∵ ROWNUM은 FROM절에서부터 존재)
-- → 해결책 : 프롬절에서 급여로 정렬 →인라인뷰를 사용
SELECT ROWNUM, EMP_NAME, SALARY
FROM (SELECT EMP_NAME, SALARY
        FROM EMPLOYEE
...
SELECT ROWNUM, * --missing expression : 받을 수 없는 문법
FROM (SELECT EMP_NAME, SALARY
        FROM EMPLOYEE
        ORDER BY SALARY DESC)
WHERE ROWNUM <=5        
...
SELECT ROWNUM, MYFROM.*
FROM (SELECT EMP_NAME, SALARY
        FROM EMPLOYEE
        ORDER BY SALARY DESC) MYFROM
 WHERE ROWNUM <=5        
ORDER BY SALARY DESC;        

-- 인라인 뷰 예시
SELECT *
FROM(SELECT  EMP_NAME 이름, SALARY, BONUS
    FROM EMPLOYEE
    WHERE EMP_NAME LIKE '%송%') -- 인라인 뷰를 통해 내가 원하는 판을 만들어줄 수 있다.
WHERE EMP_NAME = '송종기';  -- EMP_NAME COLUMN명 = 이름  
--WHERE EMP_ID = 201; -- 에러 ∵내가 가지고 있는 판은 EMP_NAME, SALARY, BONUS 만 존재
SELECT SALARY, 이름
FROM(SELECT  EMP_NAME 이름, SALARY, BONUS
    FROM EMPLOYEE
    WHERE EMP_NAME LIKE '%송%') -- 인라인 뷰를 통해 내가 원하는 판을 만들어줄 수 있다.
...
--공동순위 이후에 그다음번호가 다릅니다.
-- 사용빈도 : 인라인뷰 >>> RANK() OVER / DENSE_RANK() OVER
